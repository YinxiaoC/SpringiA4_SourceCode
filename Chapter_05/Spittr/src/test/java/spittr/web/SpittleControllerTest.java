...
public class SpittleControllerTest {
    ...
    public void shouldShowRecentSpittles() throws Exception {
        ...
        //Mock Repository
        //创建 SpittleRepository 接口的 mock 实现，这个实现会从它的 findSpittles() 方法中返回 20 个 Spittle 对象
        SpittleRepository mockRepository = mock(SpittleRepository.class);
        when(mockRepository.findSpittles(Long.MAX_VALUE, 20))
                .thenReturn(expectedSpittles);
        //将这个 Repository 注入到一个新的 SpittleController 实例中，然后创建 MockMvc 并使用这个控制器
        SpittleController controller = new SpittleController(mockRepository);
        //Mock SpringMvc
        //调用setSingleView（） mock框架不需要解析视图名称
        // （按照默认的视图解析规则时， MockMvc 就会发生失败，因为无法区分视图路径和控制器的路径。）
